...
const Button = ({ variantColor, style, ...props }: ButtonProps) => {
  const innerStyle = {
    ...style,
    color: "white",
    backgroundColor: variantColor === "primary" ? "rgb(26,100,255)" : "rgb(170,170,170)",
  };
...
const CounterButtons = ({ setCount }: CounterButtonsProps) => {
  return (
    <div>
      <Button variantColor="primary" onClick={() => setCount((count) => count - 1)}>
        -
      </Button>
...
const Tooltip = ({ children, contents, style }: ToolTipProps) => {
...
// Render Props
const MousePosition = ({ children }: { children: ({ x, y }: { x: number; y: number }) => React.ReactElement }) => {
      ...
      <MousePosition>
        {({ x, y }) => (
          <Tooltip
            ...
            contents={<span style={{ color: "red" }}>Mouse Position Tooltip</span>}
          >
            <h2>
              Mouse Position: {x}, {y}
            </h2>
